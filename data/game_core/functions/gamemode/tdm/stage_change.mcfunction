...
# === 全域廣播（聊天欄）===
execute if score #tdm_stage tdm_config matches 0 run tellraw @a ["",{"text":"[系統] ","color":"yellow","bold":true},{"text":"已進入中期階段！擊殺獎勵 ×3，重生補貼 $45","color":"yellow"}]
execute if score #tdm_stage tdm_config matches 1 run tellraw @a ["",{"text":"[系統] ","color":"red","bold":true},{"text":"已進入後期階段！擊殺獎勵 ×9，重生補貼 $135","color":"red"}]
