...
execute at @e[type=text_display,tag=br_altar,distance=..7,limit=1,sort=nearest] as @a if score @s team_id = #current_player team_id if entity @s[tag=soul_picked] run function game_core:gamemode/br/altar/revive