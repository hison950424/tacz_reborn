...
execute as @a[tag=!in_lobby,tag=!on_parkour] if score @s vanilla_death > @s minidm_death_prev run function game_core:gamemode/minidm_death_event
...
execute as @a[tag=!in_lobby,tag=!on_parkour] if score @s gd656killicon.kill > @s minidm_kill_prev run function game_core:gamemode/minidm_kill_event
