...
# 90 秒（佔領開放後 60 秒）：若本回合預定開放第二個據點，且目前只有 1 個點開啟
...
# 持點計時分（方向 B）：據點進度領先方每秒 +1 dom_score
...
# 金錢上限 $9,600（每秒強制執行，所有 DOM 玩家）
execute as @a[team=red] if score @s gd656killicon.score matches 9601.. run gd656killicon server statistics set score @s 9600
execute as @a[team=blue] if score @s gd656killicon.score matches 9601.. run gd656killicon server statistics set score @s 9600
