...
# 一般箱
execute as @e[type=marker,tag=active_loot,tag=crate_general,scores={rand_val=1..60}] at @s run setblock ~ ~ ~ minecraft:barrel{LootTable:"br:chests/general"} replace

# 醫療箱與視覺模型
execute as @e[type=marker,tag=active_loot,tag=crate_medical,scores={rand_val=1..60}] at @s run setblock ~ ~ ~ variantbarrels:cherry_barrel{LootTable:"br:chests/medical"} replace

# 配件箱與視覺模型
execute as @e[type=marker,tag=active_loot,tag=crate_attach,scores={rand_val=1..60}] at @s run setblock ~ ~ ~ variantbarrels:warped_barrel{LootTable:"br:chests/attach"} replace

# 高階箱與視覺模型
execute as @e[type=marker,tag=active_loot,tag=crate_high,scores={rand_val=1..60}] at @s run setblock ~ ~ ~ variantbarrels:bamboo_barrel{LootTable:"br:chests/high"} replace
