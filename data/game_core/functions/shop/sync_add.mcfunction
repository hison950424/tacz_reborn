...
# 執行者: @s (接收方)
# 目的: 將動態變數 #sync_val 拆解為靜態指令增加 (扁平化架構)
# ==========================================
# 處理 500
...
execute if score #sync_val temp_score matches 500.. run scoreboard players add @s gd656killicon.score 50
...
execute if score #sync_val temp_score matches 500.. run scoreboard players add @s gd656killicon.score 5
...
execute if score #sync_val temp_score matches 500.. run scoreboard players add @s gd656killicon.score 1
execute if score #sync_val temp_score matches 1..4 run scoreboard players remove #sync_val temp_score 1

# 判定是否需要再次進入迴圈
execute if score #sync_val temp_score matches 1.. run function game_core:shop/sync_add