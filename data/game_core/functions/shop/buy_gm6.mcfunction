...
clear @s tacz:modern_kinetic_gun{ShopItem:"gm6"}
...
# 6. 發放真正的武器、音效、扣款
give @s tacz:modern_kinetic_gun{GunId:"suffuse:gm6",GunFireMode:"SEMI"} 1
playsound block.anvil.use master @s ~ ~ ~ 1 1.2
gd656killicon server statistics add score @s -1400
# # 只有哨兵能買 沒有動態價格問題
# execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -610
tellraw @s ["",{"text":"[商店] 成功購買 gm6 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
