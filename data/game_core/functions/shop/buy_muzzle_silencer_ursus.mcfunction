# ==========================================
# 檔案: shop/buy_muzzle_silencer_ursus.mcfunction
# 觸發: 玩家把「假 Ursus 滅音器」拿進背包的瞬間
# ==========================================

# 0. 【核心防弊】瞬間沒收玩家剛從箱子裡拿到的「展示假物品」
clear @s tacz:attachment{ShopItem:"muzzle_silencer_ursus"}
...
# 3. 動態價格設定 (標準滅音器 $150 / 支援兵加收 25% 為 $188)
scoreboard players set @s shop_price 150
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 188
...
# 5. 發放真正的配件、音效、扣款
give @s tacz:attachment{AttachmentId:"tacz:muzzle_silencer_ursus"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -150
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -188
tellraw @s ["",{"text":"[商店] 成功購買 Ursus 滅音器 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]