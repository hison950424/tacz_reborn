# ==========================================
# 檔案: shop/buy_spas_12_tacz.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"spas_12_tacz"}
...
execute if score @s class_type matches 4 run tellraw @s {"text":"[商店] 權限拒絕：您的職業無法購買散彈槍！","color":"red"}
execute if score @s class_type matches 4 run return 0

scoreboard players set @s shop_price 550
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 688
...
give @s tacz:modern_kinetic_gun{GunId:"tacz:spas_12",GunFireMode:"SEMI"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -550
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -688
tellraw @s ["",{"text":"[商店] 成功購買 spas_12 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]