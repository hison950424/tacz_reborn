# ==========================================
# 檔案: shop/buy/scope_qlu11s.mcfunction
# ==========================================
clear @s tacz:attachment{ShopItem:"scope_qlu11s"}
...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 112
...
give @s tacz:attachment{AttachmentId:"suffuse:scope_qlu11s"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -112
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -140
tellraw @s ["",{"text":"[商店] 成功購買 QLU-11 光電 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
