...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 360
...
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -360
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -450
tellraw @s ["",{"text":"[商店] 成功購買 AA-12 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
