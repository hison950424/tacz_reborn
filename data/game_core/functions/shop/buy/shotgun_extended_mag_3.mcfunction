# ==========================================
# 檔案: shop/buy/shotgun_extended_mag_3.mcfunction
# ==========================================
clear @s tacz:attachment{ShopItem:"shotgun_extended_mag_3"}
...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 160
...
give @s tacz:attachment{AttachmentId:"tacz:shotgun_extended_mag_3"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -160
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -200
tellraw @s ["",{"text":"[商店] 成功購買 霰彈彈匣 III ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
