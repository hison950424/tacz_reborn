# ==========================================
# 檔案: shop/buy/laser_dbala2_attachment.mcfunction
# ==========================================
clear @s tacz:attachment{ShopItem:"laser_dbala2_attachment"}
...
scoreboard players set @s shop_price 90
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 72
...
give @s tacz:attachment{AttachmentId:"suffuse:laser_dbala2"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -72
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -90
tellraw @s ["",{"text":"[商店] 成功購買 DBAL-A2 雷射 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
