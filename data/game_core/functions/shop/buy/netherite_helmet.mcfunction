# ==========================================
# 檔案: shop/buy/netherite_helmet.mcfunction
# ==========================================
clear @s minecraft:netherite_helmet{ShopItem:"netherite_helmet"}
...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 192
...
give @s minecraft:netherite_helmet{Unbreakable:1b} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -192
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -240
tellraw @s ["",{"text":"[商店] 成功購買 獄髓頭盔 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
