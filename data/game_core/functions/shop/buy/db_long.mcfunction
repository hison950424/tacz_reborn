# ==========================================
# 檔案: shop/buy/db_long.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"db_long"}
...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 200
...
give @s tacz:modern_kinetic_gun{GunId:"tacz:db_long",GunFireMode:"SEMI"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -200
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -250
tellraw @s ["",{"text":"[商店] 成功購買 雙管長槍 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
