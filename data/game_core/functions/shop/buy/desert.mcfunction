# ==========================================
# 檔案: shop/buy/desert.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"desert"}
...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 144
...
give @s tacz:modern_kinetic_gun{GunId:"suffuse:desert",GunFireMode:"SEMI"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -144
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -180
tellraw @s ["",{"text":"[商店] 成功購買 Desert Eagle ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
