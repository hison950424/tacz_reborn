# ==========================================
# 檔案: shop/buy_scope_contender.mcfunction
# 觸發: 玩家把「假 Contender 4倍瞄具」拿進背包的瞬間
# ==========================================

clear @s tacz:attachment{ShopItem:"scope_contender"}
...
give @s tacz:attachment{AttachmentId:"tacz:scope_contender"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
gd656killicon server statistics add score @s -180
tellraw @s ["",{"text":"[商店] 成功購買 Contender 4x ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]