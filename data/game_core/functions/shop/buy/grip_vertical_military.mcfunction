# ==========================================
# 檔案: shop/buy/grip_vertical_military.mcfunction
# ==========================================
clear @s tacz:attachment{ShopItem:"grip_vertical_military"}
...
scoreboard players set @s shop_price 65
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 52
...
give @s tacz:attachment{AttachmentId:"tacz:grip_vertical_military"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -52
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -65
tellraw @s ["",{"text":"[商店] 成功購買 Nagoma 軍用 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
