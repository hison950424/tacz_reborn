# ==========================================
# 檔案: shop/buy/axsr.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"axsr"}
...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 800
...
give @s tacz:modern_kinetic_gun{GunId:"suffuse:axsr",GunFireMode:"SEMI"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -1000
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -1000
tellraw @s ["",{"text":"[商店] 成功購買 AXSR ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
