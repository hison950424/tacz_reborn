# ==========================================
# 檔案: shop/buy/aw50.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"aw50"}
...
scoreboard players set @s shop_price 1300
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 1040
...
give @s tacz:modern_kinetic_gun{GunId:"suffuse:aw50",GunFireMode:"SEMI"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -1040
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -1300
tellraw @s ["",{"text":"[商店] 成功購買 AW .50 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
