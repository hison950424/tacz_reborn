# ==========================================
# 檔案: shop/buy/taurus500.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"taurus500"}
...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 224
...
give @s tacz:modern_kinetic_gun{GunId:"tacz:taurus500",GunFireMode:"SEMI"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -224
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -280
tellraw @s ["",{"text":"[商店] 成功購買 Taurus 500 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
