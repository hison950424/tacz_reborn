# ==========================================
# 檔案: shop/buy/qbz_191.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"qbz_191"}
...
scoreboard players set @s shop_price 430
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 344
...
give @s tacz:modern_kinetic_gun{GunId:"tacz:qbz_191",GunFireMode:"AUTO"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -344
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -430
tellraw @s ["",{"text":"[商店] 成功購買 QBZ-191 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
