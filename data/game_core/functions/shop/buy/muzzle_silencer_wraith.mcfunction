# ==========================================
# 檔案: shop/buy/muzzle_silencer_wraith.mcfunction
# ==========================================
clear @s tacz:attachment{ShopItem:"muzzle_silencer_wraith"}
...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 80
...
give @s tacz:attachment{AttachmentId:"tacz:muzzle_silencer_wraith"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -80
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -100
tellraw @s ["",{"text":"[商店] 成功購買 魅影消音器 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
