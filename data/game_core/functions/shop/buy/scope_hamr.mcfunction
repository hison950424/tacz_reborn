# ==========================================
# 檔案: shop/buy_scope_hamr.mcfunction
# 觸發: 玩家把「假 HAMR 3x 光學瞄鏡」拿進背包的瞬間
# ==========================================

clear @s tacz:attachment{ShopItem:"scope_hamr"}
...
give @s tacz:attachment{AttachmentId:"tacz:scope_hamr"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
gd656killicon server statistics add score @s -180
tellraw @s ["",{"text":"[商店] 成功購買 HAMR 3x 瞄具 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]