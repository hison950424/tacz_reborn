# ==========================================
# 檔案: shop/buy/stock_tactical_spas_12.mcfunction
# ==========================================
clear @s tacz:attachment{ShopItem:"stock_tactical_spas_12"}
...
scoreboard players set @s shop_price 75
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 60
...
give @s tacz:attachment{AttachmentId:"tacz:stock_tactical_spas_12"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -60
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -75
tellraw @s ["",{"text":"[商店] 成功購買 佛蘭基戰術 SPAS ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
