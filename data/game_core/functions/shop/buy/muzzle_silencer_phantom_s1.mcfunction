# ==========================================
# 檔案: shop/buy/muzzle_silencer_phantom_s1.mcfunction
# ==========================================
clear @s tacz:attachment{ShopItem:"muzzle_silencer_phantom_s1"}
...
scoreboard players set @s shop_price 110
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 88
...
give @s tacz:attachment{AttachmentId:"tacz:muzzle_silencer_phantom_s1"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -88
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -110
tellraw @s ["",{"text":"[商店] 成功購買 Phantom S1 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
