# ==========================================
# 檔案: shop/buy/hk_mp5a5.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"hk_mp5a5"}
...
scoreboard players set @s shop_price 270
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 216
...
give @s tacz:modern_kinetic_gun{GunId:"tacz:hk_mp5a5",GunFireMode:"AUTO"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -216
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -270
tellraw @s ["",{"text":"[商店] 成功購買 HK MP5A5 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
