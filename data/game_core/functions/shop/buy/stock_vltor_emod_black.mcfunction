# ==========================================
# 檔案: shop/buy/stock_vltor_emod_black.mcfunction
# ==========================================
clear @s tacz:attachment{ShopItem:"stock_vltor_emod_black"}
...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 64
...
give @s tacz:attachment{AttachmentId:"suffuse:stock_vltor_emod_black"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -64
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -80
tellraw @s ["",{"text":"[商店] 成功購買 VLTOR E-MOD 黑 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
