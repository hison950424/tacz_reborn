# ==========================================
# 檔案: shop/buy/diamond_boots.mcfunction
# ==========================================
clear @s minecraft:diamond_boots{ShopItem:"diamond_boots"}
...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 128
...
give @s minecraft:diamond_boots{Unbreakable:1b} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -128
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -160
tellraw @s ["",{"text":"[商店] 成功購買 鑽石靴子 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
