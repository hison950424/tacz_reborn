# ==========================================
# 檔案: shop/buy/m107.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"m107"}
...
scoreboard players set @s shop_price 650
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 520
...
give @s tacz:modern_kinetic_gun{GunId:"tacz:m107",GunFireMode:"SEMI"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -520
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -650
tellraw @s ["",{"text":"[商店] 成功購買 M107 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
