# ==========================================
# 檔案: shop/buy/m16a1.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"m16a1"}
...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 320
...
give @s tacz:modern_kinetic_gun{GunId:"tacz:m16a1",GunFireMode:"AUTO"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -320
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -400
tellraw @s ["",{"text":"[商店] 成功購買 M16A1 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
