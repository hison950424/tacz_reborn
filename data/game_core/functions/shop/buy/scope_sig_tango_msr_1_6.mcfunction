# ==========================================
# 檔案: shop/buy_scope_sig_tango_msr_1_6.mcfunction
# 觸發: 玩家把「假 SIG Tango 1-6 速瞄具」拿進背包的瞬間
# ==========================================

clear @s tacz:attachment{ShopItem:"scope_sig_tango_msr_1_6"}
...
give @s tacz:attachment{AttachmentId:"suffuse:scope_sig_tango_msr_1_6"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
gd656killicon server statistics add score @s -300
tellraw @s ["",{"text":"[商店] 成功購買 SIG Tango 1-6x ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]