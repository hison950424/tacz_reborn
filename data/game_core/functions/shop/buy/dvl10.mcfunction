# ==========================================
# 檔案: shop/buy/dvl10.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"dvl10"}
...
scoreboard players set @s shop_price 800
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 640
...
give @s tacz:modern_kinetic_gun{GunId:"suffuse:dvl10",GunFireMode:"SEMI"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -640
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -800
tellraw @s ["",{"text":"[商店] 成功購買 DVL-10 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
