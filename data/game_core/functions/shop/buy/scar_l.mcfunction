# ==========================================
# 檔案: shop/buy/scar_l.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"scar_l"}
...
scoreboard players set @s shop_price 370
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 296
...
give @s tacz:modern_kinetic_gun{GunId:"tacz:scar_l",GunFireMode:"AUTO"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -296
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -370
tellraw @s ["",{"text":"[商店] 成功購買 SCAR-L ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
