# ==========================================
# 檔案: shop/buy/m200.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"m200"}
...
scoreboard players set @s shop_price 1550
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 1240
...
give @s tacz:modern_kinetic_gun{GunId:"suffuse:m200",GunFireMode:"SEMI"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -1240
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -1550
tellraw @s ["",{"text":"[商店] 成功購買 M200 Intervention ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
