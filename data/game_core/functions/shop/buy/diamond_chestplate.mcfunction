# ==========================================
# 檔案: shop/buy/diamond_chestplate.mcfunction
# ==========================================
clear @s minecraft:diamond_chestplate{ShopItem:"diamond_chestplate"}
...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 240
...
give @s minecraft:diamond_chestplate{Unbreakable:1b} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -240
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -300
tellraw @s ["",{"text":"[商店] 成功購買 鑽石胸甲 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
