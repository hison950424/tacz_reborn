# ==========================================
# 檔案: shop/buy/muzzle_compensator_trident.mcfunction
# ==========================================
clear @s tacz:attachment{ShopItem:"muzzle_compensator_trident"}
...
scoreboard players set @s shop_price 85
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 68
...
give @s tacz:attachment{AttachmentId:"tacz:muzzle_compensator_trident"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -68
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -85
tellraw @s ["",{"text":"[商店] 成功購買 Trident 補償器 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
