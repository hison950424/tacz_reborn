# ==========================================
# 檔案: shop/buy/ammo_mod_slug.mcfunction
# ==========================================
...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 280
...
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -280
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -350
tellraw @s ["",{"text":"[商店] 成功購買 獨頭彈 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
