# ==========================================
# 檔案: shop/buy/netherite_leggings.mcfunction
# ==========================================
clear @s minecraft:netherite_leggings{ShopItem:"netherite_leggings"}
...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 288
...
give @s minecraft:netherite_leggings{Unbreakable:1b} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -288
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -360
tellraw @s ["",{"text":"[商店] 成功購買 獄髓護腿 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
