# ==========================================
# 檔案: shop/buy/vector45.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"vector45"}
...
scoreboard players set @s shop_price 290
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 232
...
give @s tacz:modern_kinetic_gun{GunId:"tacz:vector45",GunFireMode:"AUTO"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -232
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -290
tellraw @s ["",{"text":"[商店] 成功購買 Vector .45 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
