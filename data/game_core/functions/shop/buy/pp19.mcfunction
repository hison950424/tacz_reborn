# ==========================================
# 檔案: shop/buy/pp19.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"pp19"}
...
scoreboard players set @s shop_price 310
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 248
...
give @s tacz:modern_kinetic_gun{GunId:"suffuse:pp19",GunFireMode:"AUTO"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -248
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -310
tellraw @s ["",{"text":"[商店] 成功購買 PP-19 Bizon ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
