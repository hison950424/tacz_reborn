# ==========================================
# 檔案: shop/buy/deagle_golden_long_barrel.mcfunction
# ==========================================
...
scoreboard players set @s shop_price 130
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 104
...
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -104
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -130
tellraw @s ["",{"text":"[商店] 成功購買 沙漠金鷹長槍管 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
