# ==========================================
# 檔案: shop/buy/lr_karambit.mcfunction
# ==========================================
clear @s lrtactical:melee{ShopItem:"lr_karambit"}
...
scoreboard players set @s shop_price 55
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 44
...
give @s lrtactical:melee{MeleeWeaponId:"lrtactical:karambit"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -44
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -55
tellraw @s ["",{"text":"[商店] 成功購買 卡拉姆比刀 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
