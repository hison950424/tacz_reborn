# ==========================================
# 檔案: shop/buy_muzzle_choke_sg.mcfunction
# 觸發: 玩家把「假 散彈槍喉縮」拿進背包的瞬間
# ==========================================

clear @s tacz:attachment{ShopItem:"muzzle_choke_sg"}
...
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 312
...
give @s tacz:attachment{AttachmentId:"tacz:muzzle_choke_sg"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -250
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -312
tellraw @s ["",{"text":"[商店] 成功購買 散彈槍喉縮 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]