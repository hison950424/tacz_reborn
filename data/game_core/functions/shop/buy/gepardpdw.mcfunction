# ==========================================
# 檔案: shop/buy/gepardpdw.mcfunction
# ==========================================
...
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 272
...
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -272
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -340
tellraw @s ["",{"text":"[商店] 成功購買 Gepard PDW ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
