# ==========================================
# 檔案: shop/buy/ptrd41.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"ptrd41"}
...
scoreboard players set @s shop_price 1800
execute if score @s class_type matches 1 run scoreboard players set @s shop_price 1440
...
give @s tacz:modern_kinetic_gun{GunId:"suffuse:ptrd41",GunFireMode:"SEMI"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute if score @s class_type matches 1 run gd656killicon server statistics add score @s -1440
execute unless score @s class_type matches 1 run gd656killicon server statistics add score @s -1800
tellraw @s ["",{"text":"[商店] 成功購買 PTRD-41 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
