...
# 6. 發放真正的武器、音效、扣款
execute run give @s tacz:modern_kinetic_gun{GunId:"tacz:p320",GunFireMode:"SEMI"} 1
execute at @s run execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
...
execute run tellraw @s ["",{"text":"[商店] 成功購買p320！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
