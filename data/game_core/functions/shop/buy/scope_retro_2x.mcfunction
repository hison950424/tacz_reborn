# ==========================================
# 檔案: shop/buy_scope_retro_2x.mcfunction
# 觸發: 玩家把「假 老式3倍光學瞄具」拿進背包的瞬間
# ==========================================

clear @s tacz:attachment{ShopItem:"scope_retro_2x"}
...
give @s tacz:attachment{AttachmentId:"tacz:scope_retro_2x"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
gd656killicon server statistics add score @s -180
tellraw @s ["",{"text":"[商店] 成功購買 老式3倍瞄具 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]