...
clear @s tacz:modern_kinetic_gun{ShopItem:"cz75"}
...
# 3. 動態價格設定 (支援兵價格 +25%)
scoreboard players set @s shop_price 6
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 7
...
# 6. 發放真正的武器、音效、扣款
give @s tacz:modern_kinetic_gun{GunId:"tacz:cz75",GunFireMode:"AUTO"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -6
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -7
tellraw @s ["",{"text":"[商店] 成功購買cz75！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
