# ==========================================
# 檔案: shop/buy_ak47.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"ak47"}
...
scoreboard players set @s shop_price 260
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 325
...
give @s tacz:modern_kinetic_gun{GunId:"tacz:ak47",GunFireMode:"AUTO"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -260
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -325
tellraw @s ["",{"text":"[商店] 成功購買 AK-47 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]