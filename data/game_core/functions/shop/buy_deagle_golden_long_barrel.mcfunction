# ==========================================
# 檔案: shop/buy_deagle_golden_long_barrel.mcfunction
# 觸發: 玩家把「假 沙漠金鷹長槍管」拿進背包的瞬間
# ==========================================

clear @s tacz:attachment{ShopItem:"deagle_golden_long_barrel"}
...
scoreboard players set @s shop_price 350
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 437
...
give @s tacz:attachment{AttachmentId:"tacz:deagle_golden_long_barrel"} 1
playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -350
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -437
tellraw @s ["",{"text":"[商店] 成功購買 金鷹長槍管 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]