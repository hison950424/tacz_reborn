# ==========================================
# 檔案: shop/buy_baseball_bat.mcfunction
# ==========================================
clear @s lrtactical:melee{ShopItem:"baseball_bat"}
...
scoreboard players set @s shop_price 750
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 938
...
give @s lrtactical:melee{MeleeWeaponId:"lrtactical:baseball_bat"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -750
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -938
tellraw @s ["",{"text":"[商店] 成功購買 棒球棍 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]