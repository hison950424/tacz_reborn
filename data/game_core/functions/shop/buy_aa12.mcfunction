# ==========================================
# 檔案: shop/buy_aa12.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"aa12"}
...
scoreboard players set @s shop_price 700
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 875
...
give @s tacz:modern_kinetic_gun{GunId:"tacz:aa12",GunFireMode:"AUTO"} 1
execute at @s run playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -700
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -875
tellraw @s ["",{"text":"[商店] 成功購買 aa12 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]