...
clear @s tacz:modern_kinetic_gun{ShopItem:"python"}
...
# 3. 動態價格設定 (支援兵價格 +25%)
scoreboard players set @s shop_price 230
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 288
...
# 5. 發放真正的武器、音效、扣款
give @s tacz:modern_kinetic_gun{GunId:"suffuse:python",GunFireMode:"SEMI"} 1
playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -230
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -288
tellraw @s ["",{"text":"[商店] 成功購買python！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]
