# ==========================================
# 檔案: shop/buy_m1014.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"m1014"}
...
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 300
...
give @s tacz:modern_kinetic_gun{GunId:"tacz:m1014",GunFireMode:"SEMI"} 1
playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -240
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -300
tellraw @s ["",{"text":"[商店] 成功購買 m1014 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]