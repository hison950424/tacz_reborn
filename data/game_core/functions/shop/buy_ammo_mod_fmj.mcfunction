# ==========================================
# 檔案: shop/buy_ammo_mod_fmj.mcfunction
# 觸發: 玩家把「假 全金屬被甲彈」拿進背包的瞬間
# ==========================================

# 0. 【核心防弊】瞬間沒收玩家剛從箱子裡拿到的「展示假物品」
clear @s tacz:attachment{ShopItem:"ammo_mod_fmj"}
...
# 3. 動態價格設定 (支援兵價格加收 25%)
scoreboard players set @s shop_price 900
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 1125
...
# 5. 發放真正的配件、音效、扣款
give @s tacz:attachment{AttachmentId:"tacz:ammo_mod_fmj"} 1
playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -900
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -1125
tellraw @s ["",{"text":"[商店] 成功購買 全金屬被甲彈(FMJ) ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]