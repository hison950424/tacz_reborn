# ==========================================
# 檔案: shop/buy_minigun.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"minigun"}
...
scoreboard players set @s shop_price 1200
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 1500
...
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -1200
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -1500
tellraw @s ["",{"text":"[商店] 成功購買 minigun ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]