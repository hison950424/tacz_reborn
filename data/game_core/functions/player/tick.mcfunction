...
# execute if score #arms_race_score_debug dummy matches 0..20 as @s run function game_core:gamemode/arms_race/score_debug
...
# 1. 監聽玩家點擊「開啟轉帳」按鈕
execute if score @s transfer_menu matches 1.. run function game_core:shop/open
# 2. 監聽玩家點擊「增減金額/取消」按鈕
execute if score @s transfer_act matches 1.. run function game_core:shop/calc
# 3.離開基地時，強制取消轉帳
execute if entity @s[tag=transfer_active] if score @s in_base_score matches 0 run function game_core:shop/cancel
