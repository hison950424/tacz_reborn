...
execute if score @s altar_cd matches 1.. run scoreboard players operation @s altar_cd_sec /= #20 br_sys

# 【修復 Bug 2.2】告示牌解析神技 (距離放寬至 ..10 確保抓取成功)
execute if score @s altar_cd matches 1.. at @s run setblock ~ ~-6 ~ oak_sign replace
execute if score @s altar_cd matches 1.. at @s run data modify block ~ ~-6 ~ front_text.messages[0] set value '[{"text":"【 祭壇重載中：","color":"red","bold":true},{"score":{"name":"@e[type=text_display,distance=..10,limit=1]","objective":"altar_cd_sec"},"color":"red","bold":true},{"text":"s 】","color":"red","bold":true}]'
