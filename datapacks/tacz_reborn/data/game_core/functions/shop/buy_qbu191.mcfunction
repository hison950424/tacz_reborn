# ==========================================
# 檔案: shop/buy_qbu191.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"qbu191"}
...
scoreboard players set @s shop_price 340
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 425
...
give @s tacz:modern_kinetic_gun{GunId:"suffuse:qbu191",GunFireMode:"AUTO"} 1
playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -340
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -425
tellraw @s ["",{"text":"[商店] 成功購買 QBU-191 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]