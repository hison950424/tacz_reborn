# ==========================================
# 檔案: shop/buy_spas12_suffuse.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"spas12_suffuse"}
...
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 150
...
give @s tacz:modern_kinetic_gun{GunId:"suffuse:spas12",GunFireMode:"SEMI"} 1
playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -120
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -150
tellraw @s ["",{"text":"[商店] 成功購買 spas12 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]