# ==========================================
# 檔案: shop/buy_rm277.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"rm277"}
...
scoreboard players set @s shop_price 450
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 562
...
give @s tacz:modern_kinetic_gun{GunId:"suffuse:rm277",GunFireMode:"AUTO"} 1
playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -450
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -562
tellraw @s ["",{"text":"[商店] 成功購買 RM277 ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]