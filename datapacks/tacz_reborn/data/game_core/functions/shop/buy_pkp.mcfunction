# ==========================================
# 檔案: shop/buy_pkp.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"pkp"}
...
scoreboard players set @s shop_price 560
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 700
...
give @s tacz:modern_kinetic_gun{GunId:"suffuse:pkp",GunFireMode:"AUTO"} 1
playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -560
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -700
tellraw @s ["",{"text":"[商店] 成功購買 pkp ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]