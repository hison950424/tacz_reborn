# ==========================================
# 檔案: shop/buy_g36k.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"g36k"}
...
scoreboard players set @s shop_price 320
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 400
...
give @s tacz:modern_kinetic_gun{GunId:"tacz:g36k",GunFireMode:"AUTO"} 1
playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -320
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -400
tellraw @s ["",{"text":"[商店] 成功購買 G36K ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]