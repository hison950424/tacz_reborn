# ==========================================
# 檔案: shop/buy_ks23m.mcfunction
# ==========================================
clear @s tacz:modern_kinetic_gun{ShopItem:"ks23m"}
...
execute if score @s class_type matches 3 run scoreboard players set @s shop_price 350
...
give @s tacz:modern_kinetic_gun{GunId:"suffuse:ks23m",GunFireMode:"SEMI"} 1
playsound block.anvil.use master @s ~ ~ ~ 1 1.2
execute unless score @s class_type matches 3 run gd656killicon server statistics add score @s -280
execute if score @s class_type matches 3 run gd656killicon server statistics add score @s -350
tellraw @s ["",{"text":"[商店] 成功購買 ks23m ！剩餘餘額：","color":"green"},{"score":{"name":"@s","objective":"gd656killicon.score"},"color":"yellow"},{"text":" 元。","color":"green"}]